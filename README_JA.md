...
>
> **⚠️ `npm install -g clawteam` も使用しないでください** — npm 上に同名のスクワッティングパッケージが存在します（発行者 `a9logic`、本プロジェクトとは無関係）。`clawteam --version` で "Coming Soon" と表示される場合は誤ったパッケージです。まず `npm uninstall -g clawteam` で削除してください。
